from django.conf.urls import url
from .views import *

app_name = 'kilt'
urlpatterns = [
    url(r'^$', index, name='info'),
    url(r'^bestilling/$', show_order, name='order'),
    url(r'^shop/$', shop, name='shop'),
    url(r'^admin/bestilling/$', period_overview, name='period_overview'),
    url(r'^admin/produkt/$', product_overview, name='product_overview'),
    url(r'^admin/produkt/(?P<pk>[0-9]+)/endre/$', EditProduct.as_view(extra_context={'new': False}),
        name='product_edit'),
    url(r'^admin/produkt/ny/$', NewProduct.as_view(extra_context={'new': True}), name='product_new'),
    url(r'^admin/bestilling/(?P<pk>[0-9]+)/vis/$', orders_in_period, name='order_view'),
    url(r'admin/bestilling/ny/$', NewPeriod.as_view(extra_context={'new': True}), name='period_new'),
    url(r'^admin/bestilling/(?P<pk>[0-9]+)/endre/$', EditPeriod.as_view(extra_context={'new': False}),
        name='period_edit'),
    url(r'^admin/bestilling/(?P<pk>[0-9]+)/excel$', download_period_as_excel, name='download_as_excel'),
]
